...
When the skill is active, the agent and switchboard should show a short activation line:

```text
FMC ACTIVE: Codex owns repo edits; Claude is review-only.
```

That line is the human-readable proof that the collaboration guardrails are engaged. Handoff packets generated by FMC start with this line.
